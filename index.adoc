...
| 包与导入相关    
| `package`       
| 定义当前文件所属包，程序入口必须为 `package main`                        
| `package main`（可执行程序）、`package utils`（工具包）     

|                 
| `import`       
| 导入外部包（系统包/自定义包），支持别名、多行导入                        
| `import "fmt"`、`import str "strings"`、`import ("math" "time")`             

| 声明与定义相关  
| `var`           
| 声明变量（全局/局部），支持类型推导、批量声明                           
| `var a int = 10`、`var b = 20`、`var (x bool; y float64)`                    

|                  
| `const`        
| 声明常量（编译期确定，不可修改），支持 `iota` 枚举                        
| `const Pi = 3.14`、`const (Sun = iota; Mon)`                                |           

| `type`          
| 定义自定义类型（别名、结构体、接口等）                                 
| `type MyInt int`、`type Person struct{Name string}`、`type SayHello interface{}` 
|
               
| `func`          
| 声明函数或方法（绑定到类型的函数），支持多返回值、可变参数               
| `func add(a,b int) int`、`func (p Person) Greet() string`                   
|

| 控制流语句      
| `if`/`else`     
| 条件判断，支持在 `if` 中声明变量（作用域仅限块内）                      
| `if age >=18 { ... } else { ... }`、`if res, err := fn(); err == nil { ... }`

|           
| `for`/`range`   | `for` 是唯一循环语句，`range` 用于遍历容器（数组/切片/map/通道）         
| `for i:=0;i<5;i++`、`for k,v := range slice { ... }`                       
|

|                  
| `switch`/`case` | 多分支选择，默认自动 break，支持任意类型匹配                              
| `switch day { case "周一": ... case "周二": ... }`                          
|
