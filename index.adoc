...
`if` 语句在基础语法中已经接触过，它还有一个强大的特性：可以在条件判断前包含一个初始化语句。这个初始化语句定义的变量，其作用域仅在 `if-else` 块内。
...
func main() {
    ...
    if a := len(os.Args); a > 1 {
        fmt.Println("你输入了", a-1, "个参数")
    } else {
        fmt.Println("没有检测到输入参数")
    }
...
Go 语言中只有 `for` 这一种循环结构，但它能实现多种循环模式。
...
func main() {
    ...
    // 2. 无表达式的 switch (相当于 if-else if-else)
    t := time.Now()
    switch {
    case t.Hour() < 12:
        fmt.Println("上午好!")
